...
import os
from werkzeug.utils import secure_filename  # 用於確保檔案名稱安全
...
# 確保工作目錄為應用的根目錄
os.chdir(app.root_path)  # 將工作目錄設置為 Flask 應用所在的根目錄
...
def index():
    if request.method == "POST":
        ...
        # 使用 secure_filename 確保檔案名稱是安全的
        filename = secure_filename(file.filename)

        # 儲存上傳的檔案到 uploads 資料夾
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(file_path)

        # 處理檔案並做預測
        try:
            data = pd.read_csv(file_path)  # 從保存的檔案路徑讀取資料
